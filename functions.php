...
function custom_redirect_function() {
    // Проверяем, что это страница таксономии WooCommerce
    if (is_tax('product_cat')) {
        // Получаем ID текущего термина
        $term_id = get_queried_object_id();

        // Получаем первые два продукта внутри этого термина
        $args = array(
            'post_type' => 'product',
            'posts_per_page' => 2,
            'tax_query' => array(
        ...
        if ($products->post_count === 1) {
            // Получаем URL первого продукта
            $product_url = get_permalink($products->posts[0]->ID);
            // Перенаправляем на URL продукта
